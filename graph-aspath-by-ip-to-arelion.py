...
if __name__ == "__main__":
    ...
    probe_with_alt_paths_to_valve = set()
    probe_with_alt_paths_to_blizzard = set()
    probe_with_alt_paths_to_ubisoft = set()

    validPrbMsms = {}
    for msm in msm_list:
        for result in msm:
            if result["dst_name"] == "162.254.197.36":
                if str(result["prb_id"]) in AllValidClean:
                    if str(result["prb_id"]) in validPrbMsms:
                        validPrbMsms[str(result["prb_id"])].append(result)
                    else:
                        validPrbMsms[str(result["prb_id"])] = [result]
    ...
    all_prbs = set()
    x =0
    for prb, msms in validPrbMsms.items():
        prb_id = msms[0]["prb_id"]
        msm_id = msms[0]["msm_id"]
        prb_src_addr = msms[0]["from"]
        prb_act_src = msms[0]["src_addr"]
        dst = msms[0]["dst_name"]
        timestamp = datetime.utcfromtimestamp(
                            msms[0]["stored_timestamp"]).strftime(
                                                        '%Y-%m-%d %H:%M:%S')
        if int(prb) != prb_id:
            print("Probe ID: %s" % prb_id)
            print("IP address: %s" % prb_src_addr)
        ...
        # Get shortest path between ASN 1299 and Blizzard
        ArelionIP = '62.115.151.121'
        LEVEL3IP = '213.19.200.78'
        HURRICANEIP = '184.104.193.150'

        ValveIP = '162.254.197.36'
        UbisoftIP = '5.200.20.245'
        BlizzardIP = '185.60.112.157'

        path = nx.shortest_path(IPGraphs, ArelionIP, UbisoftIP)
        for addr in path:
            if not prbSubGraph.has_node(addr):
                prbSubGraph.add_node(addr,
                                     asn=IPGraphs.nodes[addr]['asn'],
                                     probe=IPGraphs.nodes[addr]['probe'],
                                     destination=IPGraphs.nodes[addr]['destination'],
                                     time=IPGraphs.nodes[addr]['time'],
                                     msm_id=IPGraphs.nodes[addr]['msm_id'])

        path2edge = list(nx.utils.pairwise(path))

        for edge in path2edge:
            data = IPGraphs.get_edge_data(*edge)
            addrA, addrB = edge
            if not prbSubGraph.has_node(addrA):
                prbSubGraph.add_node(addrA,
            ...
            if not prbSubGraph.has_node(addrB):
                prbSubGraph.add_node(addrB,
            ...
            if len(data) == 1:
                ...
                for num_attr, attrs in data.items():
                    ...
                    if rtt < best_attr['rtt']:
                        best_attr = attrs
            if not prbSubGraph.has_edge(*edge):
                prbSubGraph.add_edge(addrA, addrB, 
                                     rtt=best_attr['rtt'],
                                     probe=best_attr['probe'],
                                     destination=best_attr['destination'],
                                     timestamp=best_attr['timestamp'],
                                     msm_id=best_attr['msm_id'])
        print("Getting path between Probe %s with IP %s in ASN %s to %s" %(prb_id, prb_src_addr, prb_asn, ArelionIP))
        try:
            prb2gsProviderPath = nx.shortest_path(IPGraphs, prb_src_addr, ArelionIP)
        except nx.exception.NodeNotFound:
            print("Nodes %s and %s  not found in graph" %(prb_src_addr, ArelionIP))
            continue
        except nx.exception.NetworkXNoPath:
            print("No path between %s and %s" %(prb_src_addr, ArelionIP))
            continue
        
        if prb2gsProviderPath:
            firstEdges = list(nx.utils.pairwise(prb2gsProviderPath))
            ...
            penultimateIPset.add(prb2gsProviderPath[-2])
            if int(prb) != prb_id:
                print("Change while building graph %s: %s" %(prb, prb_id))
            while (len(prb2gsProviderPath) - n) > 1:
                ipHop = prb2gsProviderPath[-n]
                if nx.has_path(IPGraphs, ipHop, ArelionIP):
                    alt_paths = nx.all_shortest_paths(IPGraphs, ipHop, ArelionIP)
                    for altpath in alt_paths:
                        if BlizzardIP in altpath:
                            continue

                        if UbisoftIP in altpath:
                            continue

                        if ValveIP in altpath:
                            continue
                        ...
                        #if altpath[-3] in penultimateIPset:
                        #    continue
                        ...
                        #penultimateIPset.add(altpath[-3])
                        probe_with_alt_paths_to_valve.add(prb_id)
                n += 1

        if int(prb) != prb_id:
            print("Second Probe ID: %s" % prb_id)
            print("Second IP address: %s" % prb_src_addr)

        if prb_id not in probe_with_alt_paths_to_valve:
            continue
        else:
        #if int(prb) in probe_with_alt_paths_to_valve:     
            print("Graph for Probe %s to be written" %prb)
            probe = prb
            net = Network(height="1000px", width="100%", notebook=True, directed=True)
            net.toggle_physics(True)
            net.show_buttons(filter_=True)
            net.from_nx(prbSubGraph)
            neigh_map = net.get_adj_list()
            new_map = {}
            ...
            print("Writing html for: %s" % probe)
            print("\n") 
            name= 'probe'+str(prb_id)
            net.set_edge_smooth('dynamic')
            
            net.show('/scratch/measurements/analysis/probes-to-ubisoft-via-arelion-ip-visualisation/'+probe+'.html')

    print(len(AllValidClean))
    print(len(all_prbs))
    print(x)
